...
def _summarize_product(product: models.Product) -> str:
    ...
    return f"{product.name} - price {price}, stock {product.stock_level}"
...
def _build_medicine_response(
    *,
    ...
    if rx:
        ...
        answer = (
            f"Yes — we have {medicine.name} in stock ({stock}). "
        ...
        if stock > 0:
            ...
            answer = f"Sorry — {medicine.name} is currently out of stock."
